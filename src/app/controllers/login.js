...
angular.module('flightNodeApp')
    .controller('LoginController', ['$scope', '$http', '$log', 'messenger', 'authService', 'navigationService', 'config', '$location', '$rootScope',
        function($scope, $http, $log, messenger, authService, navigationService, config, $location, $rootScope) {
            ...
            // Ensure that the API is awake by pinging it
            $http({ url: config.ping, method: 'get' })
                .then(function success(response) { console.info('ping response: ', response.data) },
                    function fail(response) { console.error('ping response: ', response) });
